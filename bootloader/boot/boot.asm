...
cbDirEnt    EQU 32
DirOff	    EQU 500h
IBMLOADSIZE equ 200			;J.K. Size of bootloader module in sectors (full bootloader)
ROM_DISKRD  equ 2
include version.inc
...
;
; Now we scan for the presence of KERNEL.ASM
; Check the first directory entry.

	MOV	DI,BX
	MOV	CX,11
	MOV	SI,OFFSET BIO		; point to "KERNEL  ASM"
	REPZ	CMPSB			; see if the same
	JZ	DoLoad			; if found, start loading
...
; NEW: Mass load all project files
	call	mass_scan_project	; Scan all directories
	push	cx			; Save file count
	call	mass_load_all_files	; Load all files into memory
	pop	cx			; Restore file count
	
; Display loading status
	push	si
	mov	si, OFFSET MASS_LOAD_MSG
	call	WRITE
	pop	si
...
;	 include ibmbtmes.inc
	...
	IF IBMCOPYRIGHT
BIO	DB	"BOOTLOADR"  ; Bootloader main file
DOS	DB	"BOOTLOADR"  ; Same file for simplicity
	ELSE
BIO	DB	"IO      SYS"
DOS	DB	"MSDOS   SYS"
	ENDIF

; Mass loading message
MASS_LOAD_MSG DB "Loading KSDOS project files...", 13, 10, 0
SYSTEM_LOAD_MSG DB "Loading system modules from SYSTEM/...", 13, 10, 0
