...
# ---------------------------------------------------------------------------
OVL_NAMES := CC MASM CSC MUSIC NET OPENGL PSYQ GOLD4 IDE AI MATRIX SYSINFO CALC COLOR JAVA PY PERL PHP VB DELPHI JS SETUP1 SETUP2
...
$(BUILD)/SETUP1.OVL: $(OVL_DIR)/setup1.ovl.asm  $(KERN_DIR)/ovl_api.asm | $(BUILD)
	@echo "[NASM] Assembling overlay SETUP1 (Installer Disk 1)..."
	$(NASM) $(OVL_FLAGS) -o $@ $<
	@echo "[OK]   SETUP1.OVL"

$(BUILD)/SETUP2.OVL: $(OVL_DIR)/setup2.ovl.asm  $(KERN_DIR)/ovl_api.asm | $(BUILD)
	@echo "[NASM] Assembling overlay SETUP2 (Installer Disk 2)..."
	$(NASM) $(OVL_FLAGS) -o $@ $<
	@echo "[OK]   SETUP2.OVL"
...
$(DISK1_IMG): $(BOOTSECT_BIN) $(KERNEL_BIN) $(BUILD)/SETUP1.OVL $(BUILD)/SETUP2.OVL | $(BUILD)
	@echo "[PERL] Building Installer Disk 1 (setup inicial)..."
	$(PERL) $(TOOLS)/mkimage.pl $(BOOTSECT_BIN) $(KERNEL_BIN) $(DISK1_IMG) \
		$(BUILD)/SETUP1.OVL $(BUILD)/SETUP2.OVL
	@echo "[OK]   disk1.img (Disco 1 - Instalador Setup Inicial)"

$(DISK2_IMG): $(BOOTSECT_BIN) $(KERNEL_BIN) $(BUILD)/SETUP2.OVL | $(BUILD)
	@echo "[PERL] Building Installer Disk 2 (setup secundário)..."
	$(PERL) $(TOOLS)/mkimage.pl $(BOOTSECT_BIN) $(KERNEL_BIN) $(DISK2_IMG) \
		$(BUILD)/SETUP2.OVL
	@echo "[OK]   disk2.img (Disco 2 - Instalador Arquivos do Sistema)"
